...
# ==================== NEW SERVICES FOR PATIENT MEDICATION MANAGEMENT ====================

def get_today_status_service(patient_id: str) -> Tuple[Dict[str, Any], int]:
    """
    Get today's medications with taken status and progress - NEW
    Returns: Unified view for Today's Overview screen
    """
    try:
        print(f"[*] Getting today's status for patient: {patient_id}")
        
        # Get patient
        patient = db.patients_collection.find_one({"patient_id": patient_id})
        if not patient:
            return jsonify({'success': False, 'message': f'Patient not found: {patient_id}'}), 404
        ...
        today_tracking = [
            t for t in patient.get('medication_daily_tracking', [])
            if t.get('date_taken') == today
        ]
        ...
        for log in medication_logs:
            if log.get('is_prescription_mode'):
                continue
                
            medication_name = log.get('medication_name')
            
            # Check each dosage
            for dosage in log.get('dosages', []):
                if dosage.get('reminder_enabled'):
                    time_str = dosage.get('time', '')
                    ...
                    for tracking in today_tracking:
                        if (tracking.get('tablet_name') == medication_name 
                            and tracking.get('tablet_taken_today') == True):
                            taken_today = True
                            taken_at = tracking.get('time_taken', '')
                            break
                    ...
                    try:
                        if time_str:
                            ...
                            if taken_today:
                                status = "Taken"
                                status_time = f"Taken at {taken_at}"
                            elif dose_time < current_time:
                                status = "Missed"
                                status_time = f"Scheduled: {time_str}"
                            elif (dose_time - current_time).total_seconds() <= 900:  # 15 minutes
                                status = "Due Soon"
                                status_time = f"Due Soon: {time_str}"
                            else:
                                status = "Scheduled"
                                status_time = f"Scheduled: {time_str}"
                        else:
        ...
        # Calculate progress
        total_medications = len(today_medications)
        taken_count = sum(1 for m in today_medications if m['taken_today'])
        pending_count = total_medications - taken_count
        
        # Group by time period
        morning_meds = [m for m in today_medications if _is_morning_time(m['time'])]
        afternoon_meds = [m for m in today_medications if _is_afternoon_time(m['time'])]
        evening_meds = [m for m in today_medications if _is_evening_time(m['time'])]
        missed_meds = [m for m in today_medications if m['status'] == 'Missed']
        ...
        return jsonify({
            ...
            'daily_progress': {
                ...
                'percentage': round((taken_count / total_medications * 100) if total_medications > 0 else 0, 1),
                'display': f"{taken_count}/{total_medications}"
            },
            'medications': today_medications,
            'grouped': {
                'missed': missed_meds,
                'morning': morning_meds,
                'afternoon': afternoon_meds,
                'evening': evening_meds
            },
...
def mark_taken_service(data: Dict[str, Any]) -> Tuple[Dict[str, Any], int]:
    """
    Mark medication as taken - NEW
    Simplified version for patient to mark as taken
    """
    try:
        patient_id = data.get('patient_id')
        medication_name = data.get('medication_name')
        ...
        if not patient_id or not medication_name or not time_str:
            return jsonify({
                'success': False,
                'message': 'patient_id, medication_name, and time are required'
            }), 400
        ...
        tracking_entry = {
            'tablet_name': medication_name,
            'tablet_taken_today': True,
            'date_taken': today,
            'time_taken': now,
            'is_prescribed': True,
            'notes': notes,
        ...
        # Add to medication_daily_tracking
        if 'medication_daily_tracking' not in patient:
            patient['medication_daily_tracking'] = []
        
        patient['medication_daily_tracking'].append(tracking_entry)
        ...
        return jsonify({
            'success': True,
            'message': f'{medication_name} marked as taken',
            'taken_at': now,
            'date': today
        }), 200
...
def update_medication_schedule_service(data: Dict[str, Any]) -> Tuple[Dict[str, Any], int]:
    """
    Update medication schedule without deleting history - NEW
    Used when doctor changes prescription
    """
    try:
        patient_id = data.get('patient_id')
        medication_name = data.get('medication_name')
        ...
        if not patient_id or not medication_name:
            return jsonify({
                'success': False,
                'message': 'patient_id and medication_name are required'
            }), 400
        ...
        for log in medication_logs:
            if log.get('medication_name') == medication_name:
                # Update dosages (schedule)
                log['dosages'] = new_schedule.get('dosages', [])
                log['last_updated'] = datetime.now().isoformat()
                updated = True
                break
        
        if not updated:
            return jsonify({'success': False, 'message': f'Medication not found: {medication_name}'}), 404
        
        # Save updated logs
        db.patients_collection.update_one(
            {"patient_id": patient_id},
            {"$set": {"medication_logs": medication_logs}}
        )
        
        return jsonify({
            'success': True,
            'message': f'Medication schedule updated successfully',
...
def snooze_reminder_service(data: Dict[str, Any]) -> Tuple[Dict[str, Any], int]:
    """
    Snooze medication reminder - NEW
    """
    try:
        patient_id = data.get('patient_id')
        medication_name = data.get('medication_name')
        ...
        return jsonify({
            'success': True,
            'message': f'Reminder for {medication_name} snoozed for {snooze_minutes} minutes',
...
def disable_medication_service(data: Dict[str, Any]) -> Tuple[Dict[str, Any], int]:
    """
    Disable medication reminders - KEEP HISTORY FOR DOCTOR TRACKING - NEW
    Preserves complete medical record for doctor review
    """
    try:
        patient_id = data.get('patient_id')
        medication_name = data.get('medication_name')
        ...
        # Find and UPDATE medication status (don't delete!)
        medication_logs = patient.get('medication_logs', [])
        updated = False
        
        for log in medication_logs:
            if log.get('medication_name') == medication_name:
                # Disable reminders only
                for dosage in log.get('dosages', []):
                    dosage['reminder_enabled'] = False
                
                # Mark as completed/stopped
                log['status'] = 'completed'
                log['last_active_date'] = datetime.now().isoformat()
                log['stopped_at'] = datetime.now().isoformat()
                log['last_updated'] = datetime.now().isoformat()
                
                # Calculate compliance for doctor
                tracking = patient.get('medication_daily_tracking', [])
                med_tracking = [t for t in tracking if t.get('tablet_name') == medication_name]
                
                total_taken = sum(1 for t in med_tracking if t.get('tablet_taken_today') == True)
                
                log['compliance_stats'] = {
                    'total_doses_taken': total_taken,
                    'total_doses_scheduled': len([d for d in log.get('dosages', []) if d.get('reminder_enabled')]),
                    'compliance_rate': round((total_taken / len(med_tracking) * 100) if med_tracking else 0, 1)
                }
        ...
        # Save: keeps all history for doctor
        db.patients_collection.update_one(
            {"patient_id": patient_id},
            {"$set": {"medication_logs": medication_logs}}
        )
        ...
        return jsonify({
            'success': True,
            'message': f'Reminders for {medication_name} disabled (history preserved for doctor)',
            'medication_name': medication_name,
            'status': 'completed',
            'compliance_stats': log.get('compliance_stats', {}),
            'doctor_tracking_preserved': True,
...
def _is_morning_time(time_str):
    """Check if time is morning (6:00 AM - 11:59 AM)"""
    try:
...
def _is_afternoon_time(time_str):
    """Check if time is afternoon (12:00 PM - 5:59 PM)"""
    try:
        hour = int(time_str.split(':')[0])
        return 12 <= hour < 18
    except:
...
def _is_evening_time(time_str):
    """Check if time is evening (6:00 PM - 5:59 AM)"""
    try:
        hour = int(time_str.split(':')[0])

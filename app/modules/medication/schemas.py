...
class GetTodayStatusSchema(Schema):
    """Schema for unified today view response"""
    date = fields.Str()
    medications = fields.List(fields.Dict())
    progress = fields.Dict()


class MarkTakenSchema(Schema):
    """Schema for marking medication as taken"""
    patient_id = fields.Str(required=True)
    medication_name = fields.Str(required=True)
...
class UpdateMedicationScheduleSchema(Schema):
    """Schema for updating medication schedule"""
    patient_id = fields.Str(required=True)
    medication_name = fields.Str(required=True)
    keep_history = fields.Bool(default=True)
    update_future_only = fields.Bool(default=True)
    new_schedule = fields.Dict(required=True)


class SnoozeReminderSchema(Schema):
    """Schema for snoozing medication reminder"""
    patient_id = fields.Str(required=True)
    medication_name = fields.Str(required=True)
    snooze_minutes = fields.Int(default=15)


class DisableMedicationSchema(Schema):
    """Schema for disabling medication"""
    patient_id = fields.Str(required=True)
    medication_name = fields.Str(required=True)

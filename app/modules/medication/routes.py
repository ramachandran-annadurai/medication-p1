...
from .services import (
    ...
    mark_taken_service,
    update_medication_schedule_service,
    snooze_reminder_service,
    disable_medication_service
)
...
# ==================== NEW PATIENT MEDICATION ENDPOINTS ====================

@medication_bp.route('/get-today-status/<patient_id>', methods=['GET'])
def get_today_status(patient_id):
    """Get today's medications with taken status and progress"""
    return get_today_status_service(patient_id)


@medication_bp.route('/mark-taken', methods=['POST'])
def mark_taken():
    """Mark medication as taken"""
    data = request.get_json()
    return mark_taken_service(data)


@medication_bp.route('/update-medication-schedule', methods=['PUT'])
def update_medication_schedule():
    """Update medication schedule without deleting history"""
    data = request.get_json()
    return update_medication_schedule_service(data)


@medication_bp.route('/snooze-reminder', methods=['POST'])
def snooze_reminder():
    """Snooze medication reminder"""
    data = request.get_json()
    return snooze_reminder_service(data)


@medication_bp.route('/disable-medication', methods=['PUT'])
def disable_medication():
    """Disable medication (preserves history for doctor tracking)"""
    data = request.get_json()
    return disable_medication_service(data)


# ==================== ALL MEDICATION ENDPOINTS NOW WIRED ====================
# Total: 25 medication endpoints (9 core patient APIs + 5 new endpoints + 11 OCR/test)
